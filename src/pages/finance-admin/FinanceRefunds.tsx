...
import { ChevronDown, MoreVertical, Search, Calendar, Eye, CheckCircle, XCircle } from "lucide-react";
import { FinancePageWrapper } from "@/components/layout/FinancePageWrapper";
import { financeData } from "@/data/financeAdminData";
import { RefundDetailsModal, type RefundTransaction } from "@/features/finance-admin/RefundDetailsModal";
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
                {refundsData.map((r) => (
                    <tr key={r.id} className="hover:bg-gray-50 transition-colors">
                        <Td className="font-medium text-gray-900">{r.transactionId || r.id}</Td>
                        <Td>{r.user.name}</Td>
                        <Td className="text-gray-500">{r.bookingId}</Td>
                        <Td className="font-medium">₦{r.amount.toLocaleString()}</Td>
                        <Td className="max-w-xs truncate text-gray-500" title={r.reason}>{r.reason}</Td>
